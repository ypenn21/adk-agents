# Feature Implementation Plan: UI Creation (OPS-11)
...
- [x] Task 2: Implement jQuery-based chat interaction logic in `interact.html` to manage persistent session IDs (`sessionStorage`), assemble the ADK JSON payload conforming to `openapi.yaml`, call `/agent/interact/` via AJAX POST, render markdown responses with `marked.js`, and display loading/disabled states.
...
- [x] Task 4: Verify Django view `interact_with_agent` in `adk_bug_ticket_agent/views.py` and route definitions in `adk_bug_ticket_agent/urls.py` and `web/urls.py` correctly serve the template on `GET` and handle chat requests on `POST`.
...
### Codebase Structure
List of files related to this feature, indicating their current responsibility and physical location:
- `adk_bug_ticket_agent/templates/adk_agent/interact.html`: Primary UI template. Currently truncated/incomplete (cut off at line 41 with unclosed `<header>` and missing conversation container, input controls, styles, and scripts). Needs full restoration and modernization.
...
- `openapi.yaml`: Defines OpenAPI 3.0.0 contract for `/agent/interact/` (`GET` returning `text/html` and `POST` taking `appName`, `userId`, `sessionId`, `newMessage` and returning `content.parts[].text` and `timestamp`).
...
1. **Django MVC Layer**:
   ...
   - On HTTP `POST`, `interact_with_agent` receives a JSON payload conforming to the Google ADK chat schema defined in `openapi.yaml`:
     ```json
     {
...
2. **Current Defect / Incompleteness**:
   - `adk_bug_ticket_agent/templates/adk_agent/interact.html` was truncated after line 40 without message container, chat input box, send button, dark mode toggle button, or JavaScript controller logic.
...
- **FOUC (Flash of Unstyled Content) & Theme Persistence**: The dark mode preference should be stored in `localStorage.getItem('adk_theme')` with fallback to `prefers-color-scheme: dark`. An inline script in `<head>` must set `document.documentElement.setAttribute('data-theme', savedTheme)` immediately before DOM render to prevent flashing.
- **XSS Prevention**: Model responses parsed by `marked.js` should ensure markdown is rendered safely without malicious script execution.
- **Asynchronous AJAX & Loading States**: During agent execution (which can take 1-5 seconds for tool invocations/search), the UI must disable the input box and send button, display an animated typing indicator, and re-enable controls on completion or error.
...
#### 2. Response Payload Schema (Returned by `/agent/interact/`)
```json
{
  "content": {
    "parts": [
      {
        "text": "Here are the open high-priority bugs:\n\n| ID | Title | Status |\n|---|---|---|\n| BUG-101 | DB connection timeout | In Progress |"
      }
    ],
    "role": "model"
  },
...
- `getOrCreateSessionId(): string`: Retrieves `adk_chat_session_id` from `sessionStorage` or creates a new UUID via `crypto.randomUUID()`.
- `toggleTheme(): void`: Toggles `data-theme` attribute between `light` and `dark`, updating `localStorage` and icon visibility.
- `appendMessage(role: 'user' | 'agent', text: string): void`: Formats and appends a chat bubble to `#conversation`, rendering Markdown via `marked.parse(text)` if role is `'agent'`.
- `sendMessage(): void`: Reads `#messageInput`, validates non-empty, calls `appendMessage('user', text)`, activates loading indicator, triggers `$.ajax`, handles response/error, and restores focus.
- `scrollToBottom(): void`: Smoothly or directly scrolls `#conversation` to `scrollHeight`.
...
2. **Changes needed**:
   ...
   - Embed CSS styles with modern design system:
     ...
     - Animated typing/loading indicator with bouncing dots.
...
5. **Clean Verification**: All automated test cases in `tests/test_ui_views.py` and existing test suites pass without errors.
